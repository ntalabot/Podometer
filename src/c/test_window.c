...
static void progress_layer_update_proc(Layer *layer, GContext *ctx) {
  ...
  // progress bar
  inset = grect_inset(layer_get_bounds(layer), GEdgeInsets(12));
  ...
  // internal circle
  inset = grect_inset(layer_get_bounds(layer), GEdgeInsets(20));
  graphics_context_set_fill_color(ctx,GColorWhite);
  graphics_fill_radial(ctx, inset, GOvalScaleModeFitCircle, 1, DEG_TO_TRIGANGLE(0), DEG_TO_TRIGANGLE(360));

  // start marker
  inset = grect_inset(layer_get_bounds(layer), GEdgeInsets(10));
  graphics_context_set_fill_color(ctx, GColorWhite);
...
static void window_layer_update_proc(Layer *layer, GContext *ctx) {
  APP_LOG(APP_LOG_LEVEL_INFO, "window layer update");
  ...
  text_layer_set_text(s_textlayer_3, buffer2);
...
static void initialise_ui(void) {
  ...
  window_set_background_color(s_window, GColorBlack);
  //layer_set_update_proc(s_window_layer, window_layer_update_proc);
  ...
  layer_add_child(s_window_layer, (Layer *)status_bar_layer);
      ...
      // progress bar
  s_progress_layer = layer_create(GRect(0, 18, 144, 150));
  ...
  layer_add_child(s_window_layer, (Layer *)s_textlayer_2);
      ...
      // goal
  s_textlayer_3 = text_layer_create(GRect(32, 95, 80, 15));
  text_layer_set_background_color(s_textlayer_3, GColorBlack);
  text_layer_set_text_color(s_textlayer_3, GColorWhite);
