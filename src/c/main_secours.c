...
// Create edges array to stock top and bottom edges separatly
  ...
  fixed1 = 33; // old value = 0.0084
  fixed2 = 100; // old value = 0.076
...
static void filter_samples(short *samples) {
  ...
  // Copy sample values in table and initialise fi
                APP_LOG(APP_LOG_LEVEL_DEBUG,"BEFORE FILTERING \n\n");
  ...
  //fix_fft(fr,fr,M,1);
                  APP_LOG(APP_LOG_LEVEL_DEBUG,"AFTER FILTERING \n\n");
...
static void accel_data_handler(AccelData *data, uint32_t num_samples) {
  ...
  { 
    raw_data[X][i+num_samples*counter] = data[i].x>>4; // >>3 is equivalent to /8
    raw_data[Y][i+num_samples*counter] = data[i].y>>4; // to avoid overflow with values that are too big
    raw_data[Z][i+num_samples*counter] = data[i].z>>4;
